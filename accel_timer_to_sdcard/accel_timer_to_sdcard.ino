...
#define GRAVITY_SCALE_FACTOR (0.0039) // 3.9mg per LSB
...
void setup(void)
{
  ...
  /* Initialize SD Card for data storage */
  do
  {
  ...
  SD.mkdir("dir/");
  myFile = SD.open("dir/accelReading.csv", FILE_WRITE);
...
int timer = 500; // 500 / 25 = 20 seconds of data at 800 Hz
...
void printFifo()
{
  ...
  for (int i = 0; i < 32; i++)
  {
    ...
    for (int k = 0; k < 5; k+=2)
    {
      ...
      myFile.print(acclVal);myFile.print(",");
    }
    myFile.println();
  }
}
