...
// The following methods are new to the Kylefruit_ADXL343 class

/**************************************************************************/
/*!
    @brief  Reads 3x16-bits from the specified register
    @param buffer the buffer to put the 3 16-bit integers into
*/
/**************************************************************************/

void Adafruit_ADXL343::read48(uint8_t *buffer) {
  Adafruit_BusIO_Register reg_obj = Adafruit_BusIO_Register(
      i2c_dev, spi_dev, AD8_HIGH_TOREAD_AD7_HIGH_TOINC, ADXL343_REG_DATAX0, 2);
      reg_obj.read(buffer, 6);
}

/**************************************************************************/
/*!
    @brief  Toggles the measurement mode of the accel
    @param measure a boolean to set the measurement on or off
*/
/**************************************************************************/

void Adafruit_ADXL343::setMeasureMode(bool measure)
{
  uint8_t regVal = readRegister(0x2D);
  if(measure)
  {
    regVal |= (1<<3);
  }
  ...
  writeRegister(0x2D, regVal);
}


/**************************************************************************/
/*!
    @brief  Sets fifo control register mode
    @param  mode the mode to set for the fifo. corresponds to the top two bits
*/
/**************************************************************************/

void Adafruit_ADXL343::setFifoMode(int mode)
{
  uint8_t regVal = readRegister(0x38);
  regVal &= 0x00111111;
  regVal |= (mode << 6);
  writeRegister(0x38, regVal);
}


/**************************************************************************/
/*!
    @brief  Sets fifo control register to number of samples
    @param intNumber number of trigger interrupt to use
    @param samples the number of samples to set the fifo to
*/
/**************************************************************************/

void Adafruit_ADXL343::setFifoParameters(uint8_t intNumber, uint8_t samples)
{
  uint8_t regVal = readRegister(0x38);
  regVal &= 0b11000000;
  regVal |= (samples-1);
